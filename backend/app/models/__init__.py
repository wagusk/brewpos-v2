...
class Table(Base):
    ...
    active: Mapped[bool] = mapped_column(Boolean, default=True)
    # M28 — section/area grouping for the table overview screen.
    # `section` is a free-form label (e.g. "Main Hall", "Patio").
    # The visible list of sections lives in settings.json so the UI
    # is fully data-driven; this column just carries the membership.
    section: Mapped[str] = mapped_column(String(40), default="Main Hall")
    # M28 — explicit sort order within a section. Lower = first.
    sort: Mapped[int] = mapped_column(Integer, default=0)
...
class Payment(Base):
    ...
    # M35 — payment processing state machine.
    # pending -> processing -> completed
    #                   \-> failed  -> (retry) -> processing
    #                   \-> cancelled
    status: Mapped[str] = mapped_column(String(20), default="pending")
    provider: Mapped[str] = mapped_column(String(40), default="mock")  # mock | stripe | etc.
    external_id: Mapped[str] = mapped_column(String(120), default="")  # provider's transaction id
    error_message: Mapped[str] = mapped_column(String(200), default="")
    amount_validated: Mapped[bool] = mapped_column(Boolean, default=False)  # amount matched bill at confirm time
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
